...
Widget sectionHeader(BuildContext context, IconData icon, String label,
    ...
    CodexSectionHeader(
      ...
      trailing: badge == null
          ...
          : Text(
              badge,
              style: Ts.style(size: Ts.xs, color: AppTokens.of(context).text3),
            ),
    );
...
Widget tileGrid(List<Widget> tiles) {
  ...
  for (var r = 0; r < tiles.length; r += cols) {
    final chunk =
        tiles.sublist(r, r + cols > tiles.length ? tiles.length : r + cols);
    final cells = <Widget>[];
    for (var i = 0; i < cols; i++) {
      if (i > 0) cells.add(const SizedBox(width: gap));
      cells
          .add(Expanded(child: i < chunk.length ? chunk[i] : const SizedBox()));
    }
    rows.add(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              ...
              // Histories are mutated in place by the poll loop. Copying keeps
              // shouldRepaint value-aware after the rolling buffer reaches cap.
              child: CustomPaint(
                painter: _SparklinePainter(List.of(data), color),
              ),
            ),
            if (footer != null) ...[
              const SizedBox(height: 6),
    ...
    if (fitReference == null) {
      ...
      valueWidget = FittedBox(
        ...
        child: Stack(children: [
          Opacity(
            opacity: 0,
            child: Text(fitReference!,
                maxLines: 1, softWrap: false, style: valueStyle),
          ),
